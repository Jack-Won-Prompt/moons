...
<form action="{{ $product->exists ? route('admin.products.update', $product) : route('admin.products.store') }}" method="POST">
    ...
    <div class="panel">
            ...
            <div class="form-grid">
                ...
                <div class="pfield">
                    ...
                    <select name="partner_id">
                        <option value="">MOONS 공식</option>
                        @foreach($partners as $p)
                            <option value="{{ $p->id }}" @selected(old('partner_id', $product->partner_id)==$p->id)>{{ $p->company_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="pfield">
                ...
                <div class="pfield">
                    <label>대표색 그라디언트 (예: #1a1a2e,#4b1248)</label>
        ...
        <div class="panel__head" style="border-top:1px solid var(--p-line);border-bottom:0;justify-content:flex-end;gap:8px">
            <a class="pbtn" href="{{ route('admin.products.index') }}">취소</a>
